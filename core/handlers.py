...
async def handle_income_expense(message: Message, state: FSMContext):
...
async def handle_amount_and_category(message: Message, state: FSMContext):
...
async def menu_history_period(callback: CallbackQuery, state: FSMContext):
...
async def menu_report_year(callback: CallbackQuery, state: FSMContext):
...
async def menu_report_month(callback: CallbackQuery, state: FSMContext):
...
async def menu_delete(message: Message, state: FSMContext):
    await message.answer(
...
async def menu_delete_period(callback: CallbackQuery, state: FSMContext):
    period = callback.data.split(":")[1]
    async with async_session() as session:
        user = await get_user_by_tg_id(session, callback.from_user.id)
        if not user:
...
async def menu_delete_record(callback: CallbackQuery, state: FSMContext):
...
# ====== Старые команды (для совместимости, не использовать) ======
