...
def notification_settings_keyboard(user) -> InlineKeyboardMarkup:
...
def debts_menu_keyboard(
    has_active: bool, has_archive: bool
) -> InlineKeyboardMarkup:
    ...
    if has_active:
        rows.append(
            [
                ...
                InlineKeyboardButton(text="📋 Карточка", callback_data="debt:view_list"),
            ]
        )
    archive_row = []
    if has_archive:
...
def debt_detail_keyboard(debt_id: int, is_archived: bool) -> InlineKeyboardMarkup:
    ...
    if not is_archived:
        rows.append(
            [InlineKeyboardButton(text="💰 Погасить", callback_data=f"debt:pay:{debt_id}")]
        )
    rows.append(
...
def debt_delete_confirm_keyboard(
    ...
    cancel_cb = f"debt:view:{debt_id}" if not is_archived else f"debt:arch_view:{debt_id}"
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
