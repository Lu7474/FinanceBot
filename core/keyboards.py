...
def goal_empty_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="➕ Создать первую цель", callback_data="goal:new")]
    ])
...
def goal_detail_keyboard(goal_id: int, is_completed: bool) -> InlineKeyboardMarkup:
    ...
    if not is_completed:
        rows.append([
            InlineKeyboardButton(text="💰 Пополнить", callback_data=f"goal:deposit:{goal_id}"),
            InlineKeyboardButton(text="📤 Снять", callback_data=f"goal:withdraw:{goal_id}"),
        ])
        rows.append([
            InlineKeyboardButton(text="✏️ Редактировать", callback_data=f"goal:edit:{goal_id}"),
        ])
        rows.append([
            InlineKeyboardButton(text="✅ Завершить", callback_data=f"goal:complete:{goal_id}"),
            InlineKeyboardButton(text="🗑 Удалить", callback_data=f"goal:delete:{goal_id}"),
        ])
        rows.append([InlineKeyboardButton(text="← Назад", callback_data="goal:list")])
    else:
        rows.append([
            InlineKeyboardButton(text="↩️ Переоткрыть", callback_data=f"goal:reactivate:{goal_id}"),
            InlineKeyboardButton(text="🗑 Удалить", callback_data=f"goal:delete:{goal_id}"),
        ])
        rows.append([InlineKeyboardButton(text="← В архив", callback_data="goal:archive")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def goal_edit_menu_keyboard(goal_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📝 Имя", callback_data=f"goal:edit_name:{goal_id}")],
        [InlineKeyboardButton(text="💰 Сумму", callback_data=f"goal:edit_amount:{goal_id}")],
        [InlineKeyboardButton(text="📅 Дедлайн", callback_data=f"goal:edit_deadline:{goal_id}")],
        [InlineKeyboardButton(text="← Назад", callback_data=f"goal:detail:{goal_id}")],
    ])
...
def goal_edit_deadline_keyboard(goal_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Убрать дедлайн", callback_data=f"goal:clear_deadline:{goal_id}")],
        [InlineKeyboardButton(text="Отмена", callback_data=f"goal:detail:{goal_id}")],
    ])
...
def goal_quick_amounts_keyboard(
    ...
    if remaining and remaining > 0:
        for pct in (10, 25, 50):
            ...
            if amount > 0:
                builder.button(text=f"{pct}% ({format_money(amount)})", callback_data=f"goal:{action}:{goal_id}:{amount}")
        if action == "qd" and monthly and monthly > 0:
            m = int(monthly)
            builder.button(text=f"Ежемес. ({format_money(m)})", callback_data=f"goal:{action}:{goal_id}:{m}")
        full = int(remaining)
        label = "Остаток" if action == "qd" else "Всё"
        builder.button(text=f"{label} ({format_money(full)})", callback_data=f"goal:{action}:{goal_id}:{full}")
...
def goal_deadline_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="Без дедлайна", callback_data="goal:no_deadline"),
...
def goal_achievement_keyboard(goal_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Завершить цель", callback_data=f"goal:complete_confirm:{goal_id}")],
...
def goal_confirm_complete_keyboard(goal_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="Да, завершить", callback_data=f"goal:complete_confirm:{goal_id}"),
                InlineKeyboardButton(text="Отмена", callback_data=f"goal:detail:{goal_id}"),
            ]
        ]
    )
...
def goal_confirm_delete_keyboard(goal_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="Да, удалить", callback_data=f"goal:delete_confirm:{goal_id}"),
                InlineKeyboardButton(text="Отмена", callback_data=f"goal:detail:{goal_id}"),
            ]
        ]
    )
...
def goal_account_keyboard(accounts: list, action: str, goal_id: int) -> InlineKeyboardMarkup:
